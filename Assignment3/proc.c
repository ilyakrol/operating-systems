...
push_to_lifo(uint va) {
    ...
    while(proc->lifo_stack.set[last] == 1) {    // search for the first empty slot
...
dequeue_scfifo() {
    ...
    while(1) {
        while (proc->fifo_queue.occupied[first] == 0) {
        ...
        proc->fifo_queue.firstIndex = (first + 1) % MAX_PSYC_PAGES;
        proc->fifo_queue.count--;
        return va;
    }
}
