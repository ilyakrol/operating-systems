...
void default_sig_handler(int sig_num);
void check_for_pending_signals(struct trapframe *tf);
...
fork(void)
{
  ...
  *np->tf = *proc->tf;
  ...
  // copy the signal handlers from the parent to the child
  for(i = 0; i < NUMSIG; i++) {
...
void
default_sig_handler(int sig_num)
{
...
{
    sighandler_t prev = proc->sig_handlers[signum]; // get the previous handler
    proc->sig_handlers[signum] = handler;   // set the new handler
    return prev;
}
...
void
check_for_pending_signals(struct trapframe *tf)
{
  int i;
  // currently in user space, proc is not not null, there is at least one pending signal
  if((tf->cs & 3) == DPL_USER && proc && proc->pending && !proc->handling_signal) {
      proc->handling_signal =1; // handling a signal
      for(i = 0; i < NUMSIG; i++) {
